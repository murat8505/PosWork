		...
		else 
		{
			try{
				...
				if(response.equalsIgnoreCase("Refund item sucessfully")){
					...
					SaveTransactionDetails.saveTransactionInDataBase(mContext,-Variables.totalBillAmount, -Variables.taxAmount, -(Variables.totalBillAmount+Variables.taxAmount), -cash,-credit, -check, -gift, -rewards,ReportsTable.YES_REFUND, ReportsTable.SUCCESSFULL,-custom1,-custom2);
